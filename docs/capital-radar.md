...
Sur la carte, taper **hgfdsq**, puis **Entrée**, sans sélectionner un champ de texte ni ouvrir une fenêtre. Répéter le code pour désactiver. Comme le code des PA illimités, il est utilisable par tout joueur qui le connaît. Aucun bouton de découverte du code n’est ajouté dans l’interface.

Le serveur mémorise `capitalRadar` sur le royaume. L’activation est conservée après reconnexion. L’API authentifiée `/api/admin/capital-radar` vérifie le code et limite les tentatives à 5 par minute.
...
Les coordonnées sont envoyées dans `enemyCapitals` uniquement aux joueurs ayant activé ce code. Il ne révèle ni terrain, ni armée, ni vision de combat, et n’autorise aucun ordre supplémentaire. Le code `ytreza` reste indépendant.
