...
| client → serveur | `chunks:subscribe` | `{ chunks: [{q,r}] }`, au plus 32 chunks, coordonnées de chunks validées.                                                                                                                                |
...
La caméra accepte un zoom de 0,2× à 3,2×, avec des pas proportionnels pour la molette et les boutons. Les abonnements couvrent le rectangle visible avec une marge ; si la limite de 32 chunks est atteinte sur un écran très large, les chunks proches du centre sont prioritaires. Le brouillard reste filtré par le serveur.
