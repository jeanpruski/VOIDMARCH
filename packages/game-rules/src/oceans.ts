...
function noise(seed: string, q: number, r: number, scale: number) {
  const x = q / scale,
    y = r / scale,
    ix = Math.floor(x),
    iy = Math.floor(y);
...
export function oceanWater(s: GameState, p: Hex): boolean {
  ...
  if (!protectedLand && s.protectedLand) {
    ...
    const buffer = 8 + noise(s.seed + 'legacy-coast', p.q, p.r, 13) * 12;
  ...
  const warpQ = (noise(s.seed + 'warp-q', p.q, p.r, 43) - 0.5) * 24;
  const warpR = (noise(s.seed + 'warp-r', p.q, p.r, 39) - 0.5) * 24;
  const value =
    noise(s.seed, p.q + warpQ, p.r + warpR, 110) * 0.7 + noise(s.seed + 'bays', p.q, p.r, 31) * 0.3;
...
export function oceanTerrain(s: GameState, p: Hex): Terrain | undefined {
  ...
  const width = noise(s.seed + 'beach', p.q, p.r, 19) > 0.5 ? 3 : 2;
